...
async function createBrowserSession(browser_args, browser_logger) {
  ...
  async function start(output) {
    ...
    async function gotoPage(u) {
      ...
      // try to bypass cookie banners by accepting all cookies
      const client = await page.target().createCDPSession()
      await client.send("Page.setDownloadBehavior", {
            behavior: "allow",
            downloadPath: "/home/wec/output"
      })
      await page.evaluate(_ => {
        ...
        elements = querySelectorAll(document, 'button, a')
        _xcc = Array.prototype.filter.call(elements, function(element){
          return RegExp('^((Alle )?akzeptieren|Verstanden|Zustimmen|Okay|OK|Allow all( cookies)?|Accept all( cookies)?)', "i").test(element.textContent.trim());
        });
        if (_xcc != null && _xcc.length != 0) {
            download(_xcc[0].outerHTML, "clicked_object.txt", "text/plain")
            _xcc[0].click();
            return _xcc[0]
        }
      });
